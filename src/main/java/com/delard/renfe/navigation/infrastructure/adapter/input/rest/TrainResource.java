...
public class TrainResource
{
    ...
    @APIResponse(responseCode = "404", description = "No trains available for the requested route/date")
    ...
    public Response getTrains(
            @Parameter(description = "Station origin (e.g., OURENSE)",
                    required = true) @QueryParam("origin") @NotBlank(message = "Origin is required") String origin,

            @Parameter(description = "Station destination (e.g., MADRID)",
                    required = true) @QueryParam("destination") @NotBlank(
                            message = "Destination is required") String destination,

            @Parameter(description = "Outbound date in format YYYY-MM-DD",
                    required = true) @QueryParam("date_out") @NotBlank(message = "Date out is required") @Pattern(
                            regexp = "\\d{4}-\\d{2}-\\d{2}",
                            message = "Date must be in format YYYY-MM-DD") String dateOut,

            @Parameter(description = "Return date in format YYYY-MM-DD (optional)") @QueryParam("date_return") @Pattern(
                    regexp = "\\d{4}-\\d{2}-\\d{2}", message = "Date must be in format YYYY-MM-DD") String dateReturn,

            @Parameter(description = "Number of adult passengers (required, must be greater than 0, max 8)",
                    required = true) @QueryParam("adults") @NotBlank(message = "Adults is required") String adults)
    {
        ...
        try {
            ...
            LOG.warnf("Train unavailability in /trains request: %s", e.getMessage());
