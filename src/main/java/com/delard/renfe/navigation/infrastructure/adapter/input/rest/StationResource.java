...
public class StationResource {
    ...
    @GET
    @Path("/stations")
    @Operation(summary = "Search stations", description = "Search for stations by text. The search is case-insensitive and matches partial text in station names.")
    @APIResponse(responseCode = "200", description = "Successful search with station results")
    @APIResponse(responseCode = "400", description = "Invalid request parameters (search text must have at least 3 characters)")
    @APIResponse(responseCode = "500", description = "Internal server error")
    public Response searchStations(
            @Parameter(description = "Search text (minimum 3 characters, case-insensitive)", required = true)
            @QueryParam("search")
